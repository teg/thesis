...
\Tom{Changed from proposition to a definition and a proposition:}
...
It should be noted that the flow associated with a derivation is not unique. However, as we will see next, it is unique up to isomorphism:
...
\begin{proof}
We sketch the proof, which follows by structural induction on $\Phi$:
\begin{itemize}
...
\Tom{Added remark, in anticipation of the local rewriting:}
...
We now show that the definition of atomic flows is `minimal', in the sense that every flow is associated with some derivation.
...
\Tom{Added remark to further justify the previous Theorem:}

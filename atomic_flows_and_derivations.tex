...
Given a derivation\/ $\Phi$ in the system $\SKS$, there is an atomic flow $\phi$ such that:
\begin{enumerate}
%
...
\TODO{Carefully check ``atom'' v.\ ``atom occurence''.}

%---------------------------------------
\begin{definition}\label{definition:AssociatedFlow}
Given a derivation $\Phi$, we say that the atomic flow $\phi$ constructed in the proof of  Proposition~\vref{proposition:FlowUnique} is the atomic flow \emph{associated with}\index{flow!associated with derivation} the derivation $\Phi$. Sometimes, when an atom occurrence $a$ in $\Phi$ is mapped to an edge $\epsilon$ in $\phi$, we decorate $a$ with the label $\epsilon$ or the label $\phi$.
\end{definition}
%---------------

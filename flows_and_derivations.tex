...
\TODO{Is it ok that the theorem does not mention the induced $\ai$-free-normal form}
...
\TODO{Revisit definition of new upper and new lower edges. Should it also refer to $\Core$?S}
...
\[
\vlder{}{}{\beta}{\vlsmallbrackets\vls((a^{\lambda'_{1,1}}_1.\cdots.a^{\lambda'_{1,k_1}}_1).\cdots.(a^{\lambda'_{n,1}}_n.\cdots.a^{\lambda'_{n,k_n}}_n).\alpha)}
\]
...
\begin{proof}
Let $\phi_1$, $\dots$, $\phi_n$ be all the non-weakly-cut-free, connected subflows of $\phi$ and, for every $1\le i\le n$, let $\lambda'_{i,1}$, $\dots$, $\lambda'_{i,k_i}$ be the new upper edges of $\phi_i$. Obtain $\Pi'$ from $\Pi$ by Lemma~\ref{LemDecompInt}:
\[
\vlder{\Pi'}{\SKS\setminus\{\aid\}}{\alpha}
{
 \vlsbr
 (
  (
   \vlinf{}{}{\vls[a_1^{\lambda'_{1,1}}.\bar a_1]}{\ttt}
  \;.\;\cdots\;.\;
   \vlinf{}{}{\vls[a_1^{\lambda'_{1,k_1}}.\bar a_1]}{\ttt}
  )
 \;.\;\cdots\;.\;
  (
   \vlinf{}{}{\vls[a_n^{\lambda'_{n,1}}.\bar a_n]}{\ttt}
  \;.\;\cdots\;.\;
   \vlinf{}{}{\vls[a_n^{\lambda'_{n,k_n}}.\bar a_n]}{\ttt}
  )
 )
}\quad,
\]
with atomic flow $\phi$. For every $1\le i\le n$ and every $1\le j\le k_i$, substitute $\vlinf{}{}{\vls[a_i^{\lambda'_{i,j}}.\bar a_i]}{\ttt}$ with $\vlsbr[a_i^{\lambda'_{i,j}}.\vlinf{}{}{\bar a_i}{\fff}]$ in $\Pi'$ to obtain a derivation with atomic flow $\Exp(\phi,\pi)$:
\[
\vlder{}{}{\alpha}
{
 \vlsbr

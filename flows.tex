...
Given two atomic flows $\phi$ and $\psi$, we write $\psi=f(\phi)$ if $\phi$ and $\psi$ are isomorphic and $f$ is a bijection from the edges of $\phi$ to the edges of $\psi$. More generally, given atomic flows $\phi_1$, $\dots$, $\phi_n$, $\psi_1$, $\dots$, $\psi_n$, we write $\psi_1=f(\phi_1)$, $\dots$, $\psi_n=f(\phi_n)$ if, for every $1\le i\le n$, $\phi_i$ and $\psi_i$ are isomorphic, and $f$ is a bijection from the edges in $\phi_1$, $\dots$, $\phi_n$ to the edges in $\psi_1$, $\dots$, $\psi_n$.
\end{definition}

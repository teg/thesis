...
\gamma_{n,1}=\cdots=\gamma_{n,n}=\fff
\]
and for every $1\le k\le n$ there exists a derivation
\[
\vlder{}{\SKS\setminus\{\aid,\aiu\}}
{
 \vls([a_1.\gamma_{k,1}].\cdots.[a_n.\gamma_{k,n}])
...
\]
Then, for every $1\le i\le n$ and every $0\le k\le n$, the formula $\gamma_{k,i}$ is true if and only if at least $k$ of the atoms $a_1$, $\dots$, $a_{i-1}$, $a_{i+1}$, $\dots$, $a_n$ are true.
\end{theorem}

%------------------------------------------------------
\begin{proof}
In this proof we will use $\theta_k\equiv\theta_k^n\avec 1n$.

We first observe that both $\theta_k\{a_i/\fff\}$ and $\theta_{k+1}\{a_i/\ttt\}$ are true if and only if at least $k$ of the atoms $a_1$, $\dots$, $a_{i-1}$, $a_{i+1}$, $\dots$, $a_n$ are true.

Hence, we can proceed by proving the following two claims:
\begin{enumerate}
 \item for every $0\le k\le n$ and for every $1\le i\le n$ we show that $\theta_k\{a_i/\fff\}\rightarrow\gamma_{k,i}$, and
 \item for every $0\le k\le n$ and for every $1\le i\le n$ we show that $\gamma_{k,i}\rightarrow\theta_{k+1}\{a_i/\ttt\}$.
\end{enumerate}

\TODO{The following observation, while true, might not be obvious. There are two options: 1) Prove it. 2) Define two classes of threshold formulae specifically for this proof so that it is true by construction (the definition would be so that the following derivations are just an equality).}

First we note that the following derivations exist for every $0\le k\le n$
\[
\vlder{}{\SKS\setminus\{\aid,\aiu\}}
{
...
{
\vls([a_1.\theta_{k+1}\{a_1/\ttt\}].\cdots.[a_n.\theta_{k+1}\{a_n/\ttt\}])
}
\]

%-----------------
We prove the implication by showing the existence of a derivation from the premiss to the conclusion. The base case is trivial:
\[
 \vlinf{=}{}
 {
  \gamma_{0,i}
...
\]
We prove the inductive case by fixing a $k\ge 1$ and assuming the following derivations exist
\[
...
\]
Since $\Phi'_{k+1,i}$ does not contain any $\aid$ instances and the premiss of $\Phi'_{k+1,i}$ does not contain any instances of $a_i$, we can apply weakening reductions to obtain:
\[
...
\]
Since $\Psi'_{k-1,i}$ does not contain any $\aiu$ instances and the conclusion of $\Psi'_{k-1,i}$ does not contain any instances of $a_i$, we can apply weakening reductions to obtain:
\[
...
\begin{remark}
We could generalise the theorem, without significantly changing the proof, by changing the condition on $\gamma_{0,i}$ and $\gamma_{n,i}$ into
\[
\gamma_{k,1}=\cdots=\gamma_{k,n}=\ttt
\quad\mbox{and}\quad
\gamma_{l,1}=\cdots=\gamma_{l,n}=\fff
\quad,
\]
for some $k<l$. Then, for every $i\ge0$, we would have that $\theta_{i-k}\rightarrow\gamma_i\rightarrow\theta_{i+n-l}$. This might serve as a starting point for finding a class of formulae, which are not threshold formulae, that will allow for smaller proofs, or alternatively to show that one does not exist.
\end{remark}
...
\]
such that, for $1\le i\le n$, $\phi_i$ is an isolated subflow that does not contain identity or cut instances. For $0\le i\le n+1$, let $\theta_i\equiv\vlsmallbrackets\vls([a_1.(\th in\avec1n)\{a_1/\fff\}].\cdots.[a_n.(\th in\avec1n)\{a_n/\fff\}])$. We define $\Phi_0$ and $\Phi_n$ as
\[
\vlder{\Phi\{\bar a_1^{\phi_1}/\ttt,\dots,\bar a_n^{\phi_n}/\ttt\}}{}
{
 \vlsbr
 [
  \beta
...
\quad\mbox{and}\quad
\vlder{\Phi\{\bar a_1^{\phi_1}/\fff,\dots,\bar a_n^{\phi_n}/\fff\}}{}
{
...
\[
\vlder{\Phi\{\bar a_1^{\phi_1}/(\th kn\avec1n)\{a_1/\fff\},\dots,\bar a_n^{\phi_n}/(\th kn\avec1n)\{a_n/\fff\}\}}{}
{
 \vlsbr
 [
  \beta

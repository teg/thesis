...
\TODO{Alessio: I was uncertain of the wording of the following definition, but it should address the above concern:}

%----------------------------------------------------
\begin{definition}\label{definiton:ReductionPolarity}
Given
\begin{itemize}
 \item a reduction $R$;
 \item two atomic flows $\phi$ and $\psi$;
 \item a polarity assignment $\pi$ for $\phi$; and
 \item isomorphisms $f_1$, $\dots$, $f_n$ between subflows $\phi_1$, $\dots$, $\phi_n$ of $\phi$ and subflows $\psi_1$, $\dots$, $\psi_n$ of $\psi$,
\end{itemize}
let $\pi_i$ be $\pi$ restricted to $\phi_i$, and let $\pi'$ be a polarity assigment for $\psi$, such that
\begin{itemize}
 \item $\pi'$ respects condition (\ref{definition:FlowReduction:item:Polarity}) of Definition~\ref{definition:FlowReduction}; and
 \item $\pi'$ restricted to $\psi_i$ equals $\pi_i$ (which is a polarity assignment for $\psi_i$ by Definition~\vref{definition:FlowIsomorphism}),
\end{itemize}
if $\pi'$ is unique, we call it $\pi$.
\end{definition}
%---------------
...
%-------------
\begin{remark}
The derivation $\Psi$ obtained in the proof of Propostion~\vref{proposition:DerivationSubstitution} is denoted $\Phi\{a^\phi/\gamma\}$. The notion of substitution can be extended to allow $\phi$ to contain interaction and cut vertices, but we shall not need that in this thesis.
\end{remark}
%-----------

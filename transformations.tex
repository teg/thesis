...
\quad,
\]
where $n$ is the number of atom occurrences in $\gamma$; moreover, the size of\/ $\Psi$ depends at most linearly on the size of\/ $\Phi$ and quadratically on the size of $\gamma$.
\end{proposition}
%----------------

%-------------------------------------------------------------------------------
\begin{proof}
We can proceed by structural induction $\Phi$. We replace every occurrence of $a^\phi$ by $\gamma$, and every inference rule instance of $\acd$, $\acu$, $\awd$ and $\awu$, where $a^\phi$ appears, by their generic counterparts with $\gamma$ in the place of $a^\phi$. The result then follows by Lemma~\vref{lemma:GenericWeakening} and Lemma~\vref{lemma:GenericContraction}.
\end{proof}
%----------
...
Let $\boldsymbol\epsilon=\{\epsilon_1,\dots,\epsilon_n\}$, $\boldsymbol\iota=\{\iota_1,\dots,\iota_m\}$, $\boldsymbol{\epsilon'}=\{\epsilon'_1,\dots,\epsilon'_n\}$ and $\boldsymbol{\iota'}=\{\iota_1,\dots,\iota_m\}$, then, when we write
\[
\atomicflow
{
(-10, 0)*{\invisiblemark};
...
\]
respectively. In other words, edges are not crossed in unexpected ways.
\end{remark}
%-----------

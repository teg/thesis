...
\TODO{Make sure we use `compatible' correctly later on.}
...
\begin{itemize}
 ...
 \item a polarity assignments $\pi$ for $\phi$ and $\pi'$ for $\psi$; and
 \item isomorphisms $f_1$, $\dots$, $f_n$ between subflows $\phi_1$, $\dots$, $\phi_n$ of $\phi$ and subflows $\psi_1$, $\dots$, $\psi_n$ of $\psi$,
\end{itemize}
we say that $\pi'$ is \emph{compatible with $\pi$}\index{polarity assignment!compatible} (\emph{with respect to $f_1$, $\dots$, $f_n$}) if, for every $1\le i\le n$ and every edge $\epsilon$ in $\phi_i$, $\pi'(f_i(\epsilon))=\pi(\epsilon)$.
\end{definition}
%---------------

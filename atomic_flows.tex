...
%----------------------------------------------------
\begin{definition}\label{definiton:InducedPolarity}
Given
\begin{itemize}
...
\end{itemize}
let $\pi_i$ be $\pi$ restricted to $\phi_i$, and let $\pi'$ be a polarity assigment for $\psi$, such that $\pi'$ restricted to $\psi_i$ equals $\pi_i$ (which is a polarity assignment for $\psi_i$ by Definition~\ref{definition:FlowIsomorphism}), then if $\pi'$ is unique, we call it $\pi$.
\end{definition}
%---------------
...
\TODO{Correct AF1, the coweakening should be a weakening.}

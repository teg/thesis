...
We will see later how a derivation containing occurences of $n$ atoms can be weakly streamlined by two applications of $\Simpl$ and $n$ applications of $\ISR$. This is the most basic procedure for obtaining a weakly streamlined derivation, in particular it only deals with one atom at a time. In the following sections we will see how we can deal with several atoms in parallell.

The operator is defined in terms of the following flow reduction.

\Tom{connected flom -> connected component}

%--------------------------------
\newcommand{\fris}{{\mathsf{is}}}
\begin{definition}\label{definition:IsolatedSubflowRemoval}
We define the reduction $\to_\fris$ (where $\fris$ stands for \emph{isolated subflow}\index{Isolated Subflow Remover!reduction}) as follows, for any flow $\phi$ and any connected component $\psi$ that does not contain identity or cut vertices:
\[
...
\Tom{Corrected remark (cf. change to Definition~\vref{definition:PathBreaker}).}
...
\Tom{Changed from itemize to enumerate to make it a bit clearer.}

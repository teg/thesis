...
In the following we refer to the names given in Definition~\ref{definition:IsolatedSubflowRemoval}. 
Given two atomic flows $\phi$ and $\psi$, such that $\phi\to_\fris\psi$ then, given an interaction (resp., cut) vertex $\nu$ in $\psi$, there is an interaction (resp., cut) vertex $\nu'$ in $\phi$, such that
\begin{enumerate}
\item $\nu=f_1(\nu')$ or $\nu=f_2(\nu')$;

...
\Tom{I considered coloring the edges of the redex as well as the contractum, but I was not sure how to do it...}
...
In the following we refer to the names given in Definition~\ref{definition:PathBreaker}. Given two atomic flows $\phi$ and $\psi$, such that $\phi\to_\frpb\psi$, then, given an interaction (resp., cut) vertex $\nu$ in $\psi$, there is an interaction (resp., cut) vertex $\nu'$ in $\phi$, such that
\begin{enumerate}

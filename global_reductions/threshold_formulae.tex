...
\Tom{Redefined ${\boldsymbol a}_l^n$ to be $(a_l,\dots,a_n)$. It is no longer a vector of $n-l$ atoms, but it is the inputs to a function of arity $n-l$.}
...
\Tom{Changed the definition to make it clear that the the names of the atoms are local parameters, so we have a wellfounded definition. This is the old definition:
...
For $n>0$ and $k\ge0$, we define the operator $\th kn$ as follows:
\[
\th kn\avec1n=
\begin{cases}
    \ttt
  ...
  \\
    ...
    \vlsbr(\th i{\overline n}\avec1{\overline n}.\th j{\underline n}\avec{\overline n+1}n)
  &
    \text{otherwise.}
\end{cases}
\]
...
For any $n>0$ and $k\ge0$, $\size{\th kn\avec1n}\le\size{\th{\overline n+1}n\avec1n}$.
\end{proposition}
...
The size of\/ $\th{\overline n+1}n\avec1n$ is $n^{\Ord{\log n}}$.
\end{lemma}

...
\Tom{atoms -> atom occurences}

\begin{proof}
We show how to construct the first derivation, the second one can be done symmetrically. We argue by induction on the number of atom occurences in $\xi\vlhole$. The base case, $\xi\vlhole=\vlhole$, is trivial and the inductive cases are:
\[
\vlderivation
{
 \vlin{=}{}
...
\quad.
\]
Since $\alpha\{a_1/\fff,\dots,a_n/\fff\}$ contains no atom occurences, there exists a derivation
\[
...
\Tom{number of atoms in $\alpha$ -> size of $\alpha$}

\begin{proof}
We show how to construct the first derivation, the second one can be done symmetrically. We argue by induction on the size of $\alpha$. We have to consider the following base case and two inductive cases:
\[
\vlinf{}{}{a}{\vls[a.a]}
\qquad\hbox{,}\qquad
\vlinf{\med}{}

...
For every $n\ge0$, we define
\begin{itemize}
\item the reduction $\to_\frqmis$ (where $\frqmis$ stands for \emph{quasipolynomial multiple isolated subflows}); and
\item the \emph{quasipolynomial multiple isolated subflow remover}, $\QMISR_n$,
\end{itemize}
as follows: Given atoms $\avec 1n$, instantiate $\to_\frmis$ and $\MISR$ with
\begin{itemize}
\item $N=n$;
...
Given atoms $a_1$, $\dots$, $a_n$, and a derivation $\Phi$ as described in Definition~\ref{definition:MultipleIsolatedSubflowsRemover},
\begin{enumerate}
\item $\QMISR_n(\Phi,a_1,\dots,a_n)$ is weakly streamlined with respect to $a$, $\dots$, $a_n$;
\item for any atom $b$,
\begin{itemize}
...
\item if $\Phi$ is on simple form with respect to $b$, then $\QMISR_n(\Phi,a_1,\dots,a_n)$ is on simple form with respect to $b$; and
\end{itemize}
\item the size of $\QMISR_n(\Phi,a_1,\dots,a_n)$ depends linearly on the size of\/ $\Phi$, and quasipolynomially on $n$.
\end{enumerate}
\end{proposition}

...
Given two atomic flows $\phi$ and $\psi$, such that $\phi\to_\frpb\psi$; let $f_1$, $f_2$, $f_3$, $g_1$, $g_2$ and $g_3$ be the isomorphisms, and let $\nu_\ai$ be the evidenced cut (resp., interaction) vertex described in Definition~\ref{definition:PathBreaker}; then, given an interaction (resp., cut) vertex $\nu$ in $\psi$, there is an interaction (resp., cut) vertex $\nu'$ in $\phi$ such that, for some $1\le i\le 3$, $\nu=f_i(\nu')$ or $\nu=g_i(\nu')$; and
\begin{itemize}
...
The \emph{Path Breaker}, $\PB$, is an operator whose arguments are an atom $a$, and a derivation with $\ai$-decomposed form 
\[
\Phi\;=\;
\vlder{\Phi'}{}
{
...
\TODO{Incomprihensible:}

We then define $\PB(\Phi,a)$ to be such that $\Psi'\to_\frpb\PB(\Phi,a)$, where $\phi$ (resp., $\psi$) in $\psi'$ is a subflow of $\psi$ (resp., $\phi$) in the redex of Definition~\vref{definition:PathBreaker}.
\end{definition}
%---------------

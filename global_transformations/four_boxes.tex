...
\TODO{Copy this phrase to the other proofs:}
...
Given an atomic flow $\phi'$ with polarity assignment $\pi$, there exists an atomic flow $\psi'$, such that $\phi'\to_\frfb\psi'$ and $\psi'$ is in simple form with respect to $\pi$.
\end{lemma}

\begin{proof}
We refer to the figure in Definition~\vref{definition:FourBoxes}. Let the subflow $\phi$ contain all the positive edges, and let the subflow $\psi$ contain all the negative edges, then $\psi'$ is in simple form with respect to $\pi$ by Definition~\vref{definition:FlowNormalForms}.
\end{proof}
%----------

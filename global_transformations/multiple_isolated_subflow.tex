...
%---------------------------------------
\begin{definition}\label{definition:MultipleIsolatedSubflowRemoval}
Given atomic flows $\gamma_0$, $\dots$, $\gamma_l$, $\delta_1$, $\dots$, $\delta_l$ that do not contain interaction or cut vertices, we define the reduction $\to_\frmis$ (where $\frmis$ stands for \emph{multiple isolated subflows}) as follows, for any atomic flow $\phi$ and any connected atomic flows $\psi_1$, $\dots$, $\psi_n$ that do not contain interaction or cut vertices:
\[
\atomicflow{
...
\begin{theorem}\label{theorem:MaybeSoundMultipleIsolatedSubflowRemoval}
...
Let
\begin{itemize}
 \item $\gamma_k$ be the atomic flow of\/ $\Gamma_k$, and 
 \item $\delta_k$ be the atomic flow consisting of, for $1\le i\le n$, one copy of $\psi_i$ for every atom occurrence in $\gamma_{i,k}$,
\end{itemize}
then
\begin{itemize}

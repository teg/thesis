...
Given an $n\ge0$, and atomic flows $\eta_0$, $\dots$, $\eta_l$, $\mu_1$, $\dots$, $\mu_l$ that do not contain interaction or cut vertices, and such that the below is a valid atomic flow, we define the reduction $\to_\frmis$ (where $\frmis$ stands for \emph{multiple isolated subflows}) as follows, for any atomic flow $\phi$ and any connected atomic flows $\psi_1$, $\dots$, $\psi_n$ that do not contain interaction or cut vertices:
\[
\atomicflow{
...
\begin{theorem}\label{theorem:MultipleIsolatedSubflowsRemovalMaybeSound}
Given $n>0$ distinct atoms $a_1$, $\dots$, $a_n$, an $N>0$, and, for $0\le k\le N$, formulae $\gamma_{k,1}$, $\dots$, $\gamma_{k,n}$, such that
\begin{itemize}
 \item $\gamma_{0,1}=\cdots=\gamma_{0,n}=\ttt$,
 \item $\gamma_{N,1}=\cdots=\gamma_{N,n}=\fff$, and
 \item for $1\le k\le N$, there exists a derivation
\[
\Gammasf_k\quad=\quad
...
Let
\begin{itemize}
 \item $\eta_k$ be the atomic flow of\/ $\Gamma_k$, and 
 \item $\mu_k$ be the atomic flow consisting of, for $1\le i\le n$, one copy of $\psi_i$ for every atom occurrence in $\gamma_{i,k}$,
\end{itemize}
then
\begin{itemize}
 \item $\to_\frmis$ is sound, and
 \item if $\phi'\to_\frmis\psi'$, then $|\psi'|/|\phi'|$ is $\Ord {N\cdot\max(|\gamma_j|)}$.
\end{itemize}
\end{theorem}

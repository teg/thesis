...
%-----------------------------------------------------
\begin{lemma}\label{lemma:IsolatedSubflowsRemovalPaths}
...
For every $n\ge0$, given the atoms, formulae and derivations described in Definition~\vref{definition:MultipleIsolatedSubflowsRemoval}, the \emph{multiple isolated subflow remover}, $\MISR_n$, is an operator whose arguments are distinct and mutually non-dual atoms $a_1$, $\dots$, $a_n$ (that, without loss of generality, we assume coincide with the atoms given in Definition~\ref{definition:MultipleIsolatedSubflowsRemoval}), and a derivation $\Phi$ that is on simple form with respect to $a_1$, $\dots$, $a_n$, such that
\[
\Phi\;=\;
\vlder{\Phi'}{}
{
